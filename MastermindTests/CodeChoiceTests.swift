...
final class CodeChoiceTests: XCTestCase {
    func test_generateOneCodeChoice() throws {
        let result = try CodeChoiceGenerator.generate(from: [.red, .blue], take: 1).options
    ...
    func test_generateTwoCodeChoices() throws {
        let result = try CodeChoiceGenerator.generate(from: [.red, .blue], take: 2).options
        XCTAssertEqual(result, [
            CodeChoice(color: .red, codeValue: 1),
    ...
    func test_notEnoughColorsIsAnError() throws {
        XCTAssertThrowsError(
            try CodeChoiceGenerator.generate(from: [.red, .blue], take: 3).options
        )
    }
}
