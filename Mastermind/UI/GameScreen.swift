            ...
            HStack {
                VStack() {
                    ForEach(1...1, id: \.self) { roundNumber in
                        RoundView(roundNumber: roundNumber, game: game, feedbackPegs: game.currentRound.feedbackPegs)
                    }
                }
                VStack {
    ...
    var body: some View {
        VStack {
            ForEach(game.codeChoices.lastToFirst, id: \.codeValue) { codeChoice in
                CodeChoiceView(codePeg: codeChoice, codeChoiceId: codeChoice.codeValue, guess: game.rounds.currentRound)
            }
        }
